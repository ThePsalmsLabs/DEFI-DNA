...
/// @dev Usage: forge script script/DeployProduction.s.sol:DeployBaseSepolia --rpc-url base_sepolia
/// --account deployer --sender <ADDRESS> --broadcast --verify
contract DeployBaseSepolia is Script {
    ...
    function run()
        public
        returns (address subscriberAddr, address readerAddr, address managerAddr)
    {
        // Verify we're on Base Sepolia
        require(block.chainid == 84_532, "Must deploy on Base Sepolia (chain 84532)");
        ...
        DNASubscriber dnaSubscriber =
            new DNASubscriber(BASE_SEPOLIA_POSITION_MANAGER, BASE_SEPOLIA_STATE_VIEW);
        ...
        DNAReader dnaReader = new DNAReader(
            BASE_SEPOLIA_POOL_MANAGER, BASE_SEPOLIA_STATE_VIEW, BASE_SEPOLIA_POSITION_MANAGER
        );
        ...
        AdvancedPositionManager advancedPosManager =
            new AdvancedPositionManager(BASE_SEPOLIA_POOL_MANAGER, BASE_SEPOLIA_POSITION_MANAGER);
    ...
    function _saveDeployment(address subscriber, address reader, address manager) internal {
        string memory json = string(
            abi.encodePacked(
                ...
                '  "dependencies": {\n',
                '    "PoolManager": "',
                vm.toString(BASE_SEPOLIA_POOL_MANAGER),
                '",\n',
                '    "PositionManager": "',
                vm.toString(BASE_SEPOLIA_POSITION_MANAGER),
                '",\n',
                '    "StateView": "',
                vm.toString(BASE_SEPOLIA_STATE_VIEW),
                '"\n',
                "  }\n",
                "}"
            )
...
/// @dev Usage: forge script script/DeployProduction.s.sol:DeployBaseMainnet --rpc-url base
/// --account deployer --sender <ADDRESS> --broadcast --verify
contract DeployBaseMainnet is Script {
        ...
        returns (address subscriberAddr, address readerAddr, address managerAddr)
    {
        ...
        require(
            address(msg.sender).balance >= 0.05 ether,
            "Insufficient ETH balance (need at least 0.05)"
        );
        ...
        // 1. Deploy DNASubscriber
        console.log("Deploying DNASubscriber...");
        DNASubscriber dnaSubscriber = new DNASubscriber(BASE_POSITION_MANAGER, BASE_STATE_VIEW);
        subscriberAddr = address(dnaSubscriber);
        console.log("  DNASubscriber deployed at:", subscriberAddr);
        ...
        console.log("Deploying DNAReader...");
        DNAReader dnaReader =
            new DNAReader(BASE_POOL_MANAGER, BASE_STATE_VIEW, BASE_POSITION_MANAGER);
        readerAddr = address(dnaReader);
        console.log("  DNAReader deployed at:", readerAddr);
        ...
        console.log("Deploying AdvancedPositionManager...");
        AdvancedPositionManager advancedPosManager =
            new AdvancedPositionManager(BASE_POOL_MANAGER, BASE_POSITION_MANAGER);
        managerAddr = address(advancedPosManager);
        console.log("  AdvancedPositionManager deployed at:", managerAddr);
    ...
    function _saveDeployment(address subscriber, address reader, address manager) internal {
        string memory json = string(
            abi.encodePacked(
                ...
                '  "dependencies": {\n',
                '    "PoolManager": "',
                vm.toString(BASE_POOL_MANAGER),
                '",\n',
                '    "PositionManager": "',
                vm.toString(BASE_POSITION_MANAGER),
                '",\n',
                '    "StateView": "',
                vm.toString(BASE_STATE_VIEW),
                '"\n',
                "  }\n",
                "}"
            )
