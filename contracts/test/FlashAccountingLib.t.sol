...
contract FlashAccountingTestHelper {
    ...
    ) external returns (FlashAccountingLib.CurrencyDelta[] memory) {
    ...
    ) external view returns (FlashAccountingLib.CurrencyDelta[] memory) {
    ...
    function calculateNetDeltas(
        FlashAccountingLib.CurrencyDelta[] memory deltas
    ) external pure returns (FlashAccountingLib.CurrencyDelta[] memory) {
...
contract FlashAccountingLibTest is Test {
    ...
    function test_CloseAndReopen_Basic() public {
        ...
        FlashAccountingLib.CurrencyDelta[] memory finalDeltas = helper.closeAndReopen(removeData, addData);
    ...
    function test_ValidateDeltas_Success() public view {
        ...
        deltas[1] = FlashAccountingLib.CurrencyDelta({
            currency: Currency.wrap(address(0x2)),
            amount: 500
        });
    ...
    function test_AreAllDeltasSettled() public view {
        FlashAccountingLib.CurrencyDelta[] memory settledDeltas = new FlashAccountingLib.CurrencyDelta[](2);
        ...
        assertTrue(
            helper.areAllDeltasSettled(settledDeltas),
            "All deltas should be settled"
        );
    }

    function test_AreAllDeltasSettled_Mixed() public view {
        FlashAccountingLib.CurrencyDelta[] memory mixedDeltas = new FlashAccountingLib.CurrencyDelta[](2);
        mixedDeltas[0] = FlashAccountingLib.CurrencyDelta({
            currency: Currency.wrap(address(0)),
            amount: 0
        });
        mixedDeltas[1] = FlashAccountingLib.CurrencyDelta({
            currency: Currency.wrap(address(0x1)),
            amount: 100
        });

        assertFalse(
            helper.areAllDeltasSettled(mixedDeltas),
